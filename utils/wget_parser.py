        ...
        for line in f:
            ...
            if "used" in json_object:
                ...
                for uid in used:
                    ...
                    # # 1. 带统计信息的格式 (if CONSOLE_ARGUMENTS.stats)
                    # output.write(
                    #     "{}\t{}\t{}:{}:{}:{}:{}\n".format(
                    #         srcUUID,           # 源节点ID
                    #         dstUUID,           # 目标节点ID
                    ...
                    #         edgetype,         # 边的类型
                    #         timestamp,        # 逻辑时间戳
                    #         adjusted_ts       # 调整后的时间戳（相对于最小时间戳）
                    #     )
                    # )

                    # # 2. 带jiffies信息的格式 (elif CONSOLE_ARGUMENTS.jiffies)
                    # output.write(
                    #     "{}\t{}\t{}:{}:{}:{}:{}\n".format(
                    #         srcUUID,           # 源节点ID
                    #         dstUUID,           # 目标节点ID
...
# 其中每条边edge的结构为：
# edge = [
#   source_id,          # 边的源节点ID (index=0)
#   dest_id,           # 边的目标节点ID (index=1)
#   source_node_type,  # 源节点类型 (index=2) 例如："process", "file"等
#   dest_node_type,    # 目标节点类型 (index=3)
#   edge_type,         # 边的类型 (index=4) 例如："used", "wasGeneratedBy"等
#   edge_order         # 边的时序信息 (index=5) 用于确定边的先后顺序
# ]
#
# 示例：
# graph = [
#   ["123", "456", "process", "file", "used", "1000"],
#   ["456", "789", "file", "process", "wasGeneratedBy", "1001"]
# ]
#
# threshold用于控制返回的边数，如果图太大（边数量超过threshold），只保留前 threshold 条边
...
if __name__ == "__main__":
    ...
    args.noencode = False
    args.stats_file = '../data/wget/stats/'
